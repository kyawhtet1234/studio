...
const affordabilityAnalysisFlow = ai.defineFlow(
    ...
    async (input) => {
        ...
        Historical Sales:
        ${input.sales.map(s => `${s.date.toISOString().substring(0,10)}: ${s.amount.toFixed(2)}`).join('\n')}

        Historical Expenses:
        ${input.expenses.map(e => `${e.date.toISOString().substring(0,10)}: ${e.amount.toFixed(2)}`).join('\n')}
